...
See for detailed list of user stories: `${SKILL_DIR}/references/user-stories.md`
Their associated sample prompts: `${SKILL_DIR}/references/sample-prompts.md`
...
- PROJ_DIR -> root directory of the the current project
- SKILL_DIR -> directory of this skill file
- ISO_TS -> !`date -u +"%Y%m%d-%H%M%S"`
- ENG_TYPE -> Which type of English should be used?
  - Has the user explicitly specified the use of American English?
  - Otherwise default to British English
  - Do *not* infer/guess this
- FILES -> Which files should be checked?
...
Based on ENG_TYPE do one of the following.
...
- Single quotation marks: Use only when there needs to be a quotation within another quotation, and only when absolutely needed. By default, avoid using them, and prefer double quotation marks.
- Bold: Use for emphasis of terms.
- Italics: Use for emphasis of thoughts/ideas.
- Numbers: For 1 to 9 as one to nine, e.g. `seven`. For 10 and above as numerals, e.g. `11`. For 1000 and above use commas per power of 1000, e.g. `1,234,567`.
- Unordered lists: Must have 2 or more items. Don't use periods unless they are full sentences. Paragraph preceding should end with a colon. Ensure empty line between preceding paragraph and first list item.
...
- 1st pass:
  - Create a list of suggested edits that use natural alternatives
...
Rearrange them:
- 1st: group all issues present in the same source file together
- 2nd: arrange the issues in order of line number (per source file)
- 3rd: merge issues the occur within same paragraph (same or adjacent line numbers) into a single issue
  - Optionally, use multiple diffs within the same issue, only if this aides with clarity; otherwise default to a single diff.

Save the report to `${PROJ_DIR}/.devrel-skills-outputs/devrel-docs-style--report-${ISO_TS}.md`
For example `/path/to/proj/.devrel-skills-outputs/devrel-docs-style--report-20260326-132726.md`
Use format: `${SKILL_DIR}/assets/dev-docs-report-template.md`
...
See `${SKILL_DIR}/references/credits.md`
